import React from 'react';

const Duc = () => {
  return (
    <div className='bg-slate-100 h-[800px]'>
      <div className='text-center font-bold mt-[40px] text-2xl text-gray-700'>
        <h1>Sign in or create an account</h1>
      </div>
      <div className='text-center'>
        Your can sign in using your Trip2 account accsess our servicesservices
      </div>
      <div className='text-center ml-[480px] mt-2 relative inline-block'>
  <span className='absolute -top-1 ml-[40px] transform -translate-x-1/2 text-red-500 text-sm'>*</span>
  Email
</div>
<div className="flex w-full mt-2 ml-[480px] text-xl max-w-2xl border border-blue-500 rounded-md">
  <input
    type="email"
    placeholder="YourEamilName@gmail.com"
    className="flex-1 w-full px-4 py-2 focus:outline-none"
  />
</div>

          <div>
          <button className="w-[670px] py-2 ml-[480px] font-semibold mt-4 text-white bg-blue-500 transition-colors border border-white  whitespace-nowrap">
            Continue with Email
          </button>
        </div>
        <div className='flex justify-items-center '> 
          <h1 className='ml-[700px] mt-3 '>Don't Have an Acount ?</h1>
          <button className='ml-[10px] mt-3 text-blue-600 underline underline-offset-1'>
  Sign Up
</button>
        </div>
        <div className='ml-[730px] mt-3'>or sign in with

        </div>
        <div className='flex items-center'>
        <button className='h-[50px] w-[50px] ml-[720px] mt-3 bg-green-600 flex justify-center items-center'>
  <img src="data:image/png;base64," alt="Google" className="h-full w-full" />
</button>
        
<button className='h-[50px] w-[50px] ml-[30px] mt-3 bg-green-600 flex justify-center items-center'>
  <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSQNAgnzw9V4JgQnIyMiXfGf0gQWTkL_DgZhA&s" alt="Facebook" className="h-full w-full" />
</button>

        </div>
        <div className='ml-[670px] mt-3'>
          By signing in car or creating an acount 
        </div>
        <div className='ml-[630px] mt-1'>
  you agree our <span className="underline text-gray-500-600">Terms & Conditions</span> and Privacy Statement
</div>
<div className='ml-[720px] mt-3'>
        All rights reserved
        </div>
        <div className='ml-[670px] mt-1'>
          Copright (2006-2024)-Trip2.com
        </div>


    </div>
  );
};

export default Duc;
