import React from 'react'
import GridW from './GridW'

const Favority = () => {
    return (
        <div className='flex w-full items-center flex-col  h-auto bg-[#F4F5F5]'>

            <div className='flex flex-col w-11/12 lg:w-3/4'>

                <div className='flex justify-between w-full pt-20'>
                    <div className='flex'>
                        <p className='font-bold text-[35px] text-[#FEB723]'>All-time Favorite Deals <span className='text-[#0081CE]'>in the Heritage Wonder</span></p>
                    </div>
                    <button className='flex items-center px-4 py-4 bg-[#0081CE] w-[150px] h-12 mt-4 rounded-lg'>
                        <span className="text-white text-[16px] font-medium">
                            Explore All <span className="ml-2">-&gt;</span>
                        </span>
                    </button>
                </div>


                <GridW />
            </div>
        </div>
    )
}

export default Favority
